...
- **UUID 항목 참조**: 항목을 `Group/Path` 대신 KeePass의 영구 UUID로 참조할 수 있어, 항목 이름을 바꾸거나 다른 그룹으로 옮겨도 그 항목을 가리키는 토큰이 깨지지 않습니다. 기존 경로 기반 토큰은 그대로 동작합니다.
- **어디서나 자동완성**: `Insert Secret` 모달, 노트에 `{{sp:` 를 직접 타이핑할 때, `safe-passage` 코드 블록 안에서까지 — 항목 후보(제목 + 전체 경로)가 자동완성으로 뜹니다. 대상 프로필이 잠겨 있으면 그 자리에서 바로 잠금을 해제할 수 있습니다.
...
- **잠금 해제 상태**: 항목의 전체 경로를 보여주는 마스킹된 칩으로 표시됩니다. 예: `Finance/API/Stripe (Password)`. 클릭하면 값이 클립보드에 복사됩니다.
...
프로필 뒤에 오는 부분은 경로(`Group/SubGroup/Title`)이거나, `uuid:` 접두사가 붙은 KeePass 항목의 UUID일 수 있습니다:

```
{{sp:work-db/uuid:Yhz3AjkUmk+HQu5+w2xdWQ==#Password}}
```

UUID는 항목 이름이 바뀌거나 다른 그룹으로 옮겨져도 절대 변하지 않으므로, UUID 참조는 데이터베이스를 재구성해도 그대로 유지되는 반면 경로 참조는 깨집니다. 기존에 저장된 경로 기반 토큰은 그대로 계속 동작합니다 — 별도로 마이그레이션할 필요는 없습니다.

### 3. 자동완성

UUID를 직접 손으로 입력할 사람은 없으므로, 참조를 입력하는 곳마다 SafePassage가 항목을 추천해 줍니다:

- **Insert Secret** 모달의 항목 입력 필드는 타이핑하는 대로 기존 항목(제목 + 전체 경로)을 추천합니다.
- 노트에 직접 `{{sp:` 를 타이핑하면 프로필 → 항목 → 필드 순서로 단계적 자동완성이 이어지며, 다음 구분자까지 자동으로 입력됩니다. 대상 프로필이 잠겨 있으면 그 자리에서 바로 잠금을 해제할 수 있는 항목이 후보로 뜹니다.
- `safe-passage` 코드 블록의 `profile:` 필드와 `entries:` 리스트 항목에도 동일한 자동완성이 적용됩니다.
...
```
```
이렇게 하면 각 필드에 대한 열과 항목별 복사 버튼이 포함된 세련된 표가 렌더링됩니다. 항목은 경로 참조와 UUID 참조를 자유롭게 섞어 쓸 수 있습니다.
...
4. **[Save]** 를 클릭합니다. 자격 증명이 실제 `.kdbx` 파일에 바로 저장되며, 커서 위치에 그 항목의 UUID 참조를 사용하는 `` `{{sp:work-db/Database/MySQL#Password}}` `` 토큰이 자동으로 삽입됩니다.
